...
export class ModuleMiddleware {
  ...
  public IsModuleExists = async (req: any, res: Response, next: () => void) => {
    ...
    const module: boolean = await My.first(Tables.MODULES, ["id"], `${idWhere} AND portal_id = ? AND name = ? AND status=1 AND deleted=0`, [req.body.portal_id , req.body.name]);


    if (module) {      
        return res.json(ResponseBuilder.badRequest( req.t("MODULE_EXISTS")));
  ...
  public checkDBDependency = async (req: any, res: Response, next: () => void) => {
    const dbDependency = await this.utils.checkDbDependency(req, res, Tables.MENUS, Tables.RIGHTS, "name", "slug", `module_id`);
