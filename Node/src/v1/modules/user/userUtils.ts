...
export class UserUtils {
  ...
  public async getUserByName(fName: string,lName:string): Promise<ResponseBuilder> {
    const email = await Sql.query("SELECT email FROM " + Tables.USERS + " WHERE first_name LIKE ? AND last_name LIKE ?", [fName,lName]);
   console.log(email);
   
    return ResponseBuilder.data(email);
  }
   ...
   */
  public async updatePasswordByEmail(password, email) {
    const user = await Sql.update(Tables.USERS, { password: password, updated_by: email }, "email = ?", [email]);
    console.log(user);

    return ResponseBuilder.data({data:user});
  }
