...
export class UserController {
  ...
  public confirm_password = async (req: any, res: Response) => {
    const password = await Utils.encryptText(req.body.password);
    ...
    console.log(await bcrypt.compare(email,tokenInfo.device_token.email))

    console.log('----')



    const result: ResponseBuilder = await this.userUtils.updatePasswordByEmail(password, email);
    const response = ResponseBuilder.getMobileSuccessResponse({}, req.t("PASSWORD_UPDATED"));
    res.status(200).json(response);
  }
