...
export class UserUtils {
  ...
  public async getUserByName(fName: string,lName:string): Promise<ResponseBuilder> {
    const user = await Sql.query("SELECT * FROM " + Tables.USERS + " WHERE first_name LIKE ? AND last_name LIKE ?", [fName,lName]);
   console.log(user);
  ...
  public async getUserAllDetails(email: string): Promise<ResponseBuilder> {
    const user = await Sql.query("SELECT email FROM " + Tables.USERS + " WHERE email LIKE ?", [email]);
    return ResponseBuilder.data(user);
  }
